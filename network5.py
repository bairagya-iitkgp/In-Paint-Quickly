...
class Network:
    def __init__(self, x, mask, points, local_x, is_training, batch_size):
        ...
        self.local_completionG = tf.reshape(self.local_completionG, [batch_size, 128, 128, 3]) 
        ...
        self.g_loss = self.calc_g_loss(x, self.completion)/self.batch_size
    ...
    def generator(self, x, is_training):
        with tf.variable_scope('generator'):
            ...
            with tf.variable_scope('conv2'):
                x = conv_layer_depth(x, [3, 3, 64, 1], [1, 1, 64, 128], 2)
                x = batch_normalize(x, is_training)
                x = tf.nn.relu(x)
            with tf.variable_scope('conv3'):
                x = conv_layer_depth(x, [3, 3, 128, 1], [1, 1, 128, 128], 1)
                x = batch_normalize(x, is_training)
                x = tf.nn.relu(x)
            with tf.variable_scope('conv4'):
                x = conv_layer_depth(x, [3, 3, 128, 1], [1, 1, 128, 256], 2)
                x = batch_normalize(x, is_training)
                x = tf.nn.relu(x)
            with tf.variable_scope('conv5'):
                x = conv_layer_depth(x, [3, 3, 256, 1], [1, 1, 256, 256], 1)
                x = batch_normalize(x, is_training)
                x = tf.nn.relu(x)
            ...
            with tf.variable_scope('dilated1'):
                x = dilated_sep_conv_layer(x, [3, 3, 256, 1], [1, 1, 256, 256], 2)
                x = batch_normalize(x, is_training)
                x = tf.nn.relu(x)
            with tf.variable_scope('dilated2'):
                x = dilated_sep_conv_layer(x, [3, 3, 256, 1], [1, 1, 256, 256], 4)
                x = batch_normalize(x, is_training)
                x = tf.nn.relu(x)
            with tf.variable_scope('dilated3'):
                x = dilated_sep_conv_layer(x, [3, 3, 256, 1], [1, 1, 256, 256], 8)
                x = batch_normalize(x, is_training)
                x = tf.nn.relu(x)
            with tf.variable_scope('dilated4'):
                x = dilated_sep_conv_layer(x, [3, 3, 256, 1], [1, 1, 256, 256], 16)
                x = batch_normalize(x, is_training)
                x = tf.nn.relu(x)
            ...
            with tf.variable_scope('deconv1'):
                x = deconv_layer_BL_sep(x, [4, 4, 256, 1], [1, 1, 256, 128], 2)
                x = batch_normalize(x, is_training)
                x = tf.nn.relu(x)
            ...
            with tf.variable_scope('deconv2'):
                x = deconv_layer_BL_sep(x, [4, 4, 128, 1], [1, 1, 128, 64], 2)
                x = batch_normalize(x, is_training)
                x = tf.nn.relu(x)
            with tf.variable_scope('conv10'):
                x = conv_layer_depth(x, [3, 3, 64, 1], [1, 1, 64, 32], 1)
                x = batch_normalize(x, is_training)
                x = tf.nn.relu(x)
            with tf.variable_scope('conv11'):
                x = conv_layer_depth(x, [3, 3, 32, 1], [1, 1, 32, 3], 1)
                x = tf.nn.tanh(x)
        ...
        def global_discriminator(x):
            ...
            with tf.variable_scope('global'):
                ...
                with tf.variable_scope('conv5'):
                    ...
                    x = batch_normalize(x, is_training)
                    x = tf.nn.relu(x)
                with tf.variable_scope('conv6_new_var'):
                    x = conv_layer(x, [5, 5, 512, 512], 2)
                    x = batch_normalize(x, is_training)
                    x = tf.nn.relu(x)
        ...
        def local_discriminator(x):
            ...
            with tf.variable_scope('local'):
                ...
                with tf.variable_scope('conv4'):
                    ...
                    x = batch_normalize(x, is_training)
                    x = tf.nn.relu(x)
                with tf.variable_scope('conv5_new_var'):
                    x = conv_layer(x, [5, 5, 512, 512], 2)
                    x = batch_normalize(x, is_training)
                    x = tf.nn.relu(x)
                with tf.variable_scope('fc'):
